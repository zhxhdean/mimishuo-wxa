...
const { code } = require('./error_code.js')
// 请求的域名
    ...
    success: function(loginRes) {
      ...
      Taro.getUserInfo({
        ...
        success: function(data) {
          ...
          // 取到用户经纬度
          let userLocation = Taro.getApp().globalData.userLocation
          if (!userLocation) {
            Taro.getLocation({
              type: 'wgs84',
              altitude: true,
              success(res) {
                ...
                Taro.getApp().globalData.userLocation = {
                  latitude: latitude,
                  longitude: longitude
                }
                userLocation = Taro.getApp().globalData.userLocation
              }
            })
          }
      ...
      success(res) {
        ...
        if (res.statusCode === 200) {
          if (res.data.result === 'success') {
            ...
            if (
              res.data.errorCode == '10002' ||
              res.data.errorCode === '10006'
            ) {
              // 如果是tonken过期，自动刷新登录接口
              // userLogin()
